...
const SectionPage = ({ title, subtitle, breadcrumb, heroImage, overview, curriculum, excellence, gallery }: SectionPageProps) => {
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="animate-fade-in">
            {activeTab === "overview" && (
              <div className="grid gap-10 lg:grid-cols-3">
                ...
                <div className="rounded-xl bg-card p-6 shadow-card">
                  <h3 className="mb-4 text-lg font-bold text-foreground font-serif">Quick Facts</h3>
                  <div className="space-y-4">
                    {overview.highlights.map((h) => (
                      <div key={h.label} className="flex items-center justify-between border-b border-border pb-2 last:border-0">
            ...
            {activeTab === "curriculum" && (
              <div>
                <p className="mb-8 max-w-3xl leading-relaxed text-muted-foreground">{curriculum.description}</p>
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {curriculum.subjects.map((subject) => (
                    <div key={subject.name} className="rounded-lg border border-border bg-card p-5 shadow-card">
                      <h4 className="mb-2 font-semibold text-foreground font-serif">{subject.name}</h4>
            ...
            {activeTab === "excellence" && (
              <div>
                <p className="mb-8 max-w-3xl leading-relaxed text-muted-foreground">{excellence.description}</p>
                <div className="grid gap-4 sm:grid-cols-2">
                  {excellence.achievements.map((achievement, i) => (
                    <div key={i} className="flex items-start gap-3 rounded-lg bg-card p-4 shadow-card">
            ...
            {activeTab === "gallery" && (
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {gallery.map((img, i) => (
                  <div key={i} className="aspect-[4/3] overflow-hidden rounded-lg shadow-card">
