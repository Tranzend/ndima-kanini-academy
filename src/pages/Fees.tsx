...
const feesData = [
  {
    ...
    includes: ["Tuition", "Learning materials", "Snacks & lunch (day)", "Meals & accommodation (boarding)", "Activity fees"],
  },
  {
    section: "Lower Primary (Grade 1-3)",
    daily: "Day: On request",
    boarding: "Boarding: On request",
    includes: ["Tuition", "Books & workbooks", "Lunch (day) / Meals (boarding)", "ICT levy", "Co-curricular activities"],
  },
  {
    section: "Upper Primary (Grade 4-6)",
    daily: "Day: On request",
    boarding: "Boarding: On request",
    includes: ["Tuition", "Books & workbooks", "Lunch (day) / Meals (boarding)", "Supervised prep (boarders)", "Exam & revision materials"],
  },
];
...
const additionalFees = [
  ...
  { item: "School Transport (per term, day scholars)", amount: "Route-dependent" },
      ...
      <section className="py-16 md:py-20">
        <div className="container mx-auto px-4">
          ...
          <div className="mb-10 flex flex-wrap justify-center gap-4">
            ...
            <button
              onClick={() => alert("The downloadable fees structure PDF will be available soon. Please call us on " + SCHOOL.primaryPhoneDisplay + " for current rates.")}
          ...
          <p className="mx-auto mb-10 max-w-2xl text-center text-sm text-muted-foreground">
            Specific fee amounts vary by class and day/boarding option. For the most up-to-date figures, please contact
            our school office or visit us in Karatina — we'll be happy to share the full structure.
          </p>
              ...
              <div key={fee.section} className="overflow-hidden rounded-xl bg-card shadow-card">
                ...
                <div className="p-6">
                  <div className="mb-3 rounded-lg bg-primary/5 p-3 text-center">
                    <p className="text-sm font-semibold text-foreground">{fee.daily}</p>
                  </div>
                  <div className="mb-6 rounded-lg bg-secondary/10 p-3 text-center">
                    <p className="text-sm font-semibold text-secondary">{fee.boarding}</p>
                  </div>
          ...
          <div className="mt-10 mx-auto max-w-2xl rounded-lg border border-secondary/30 bg-secondary/5 p-5">
            ...
            <ul className="space-y-1 text-sm text-muted-foreground">
              ...
              <li>• Sibling discounts may apply — please enquire at the office</li>
              <li>• Payment plans are available upon request</li>
              <li>• All fees are subject to annual review by the school management</li>
              <li>• Call <a href={`tel:${SCHOOL.primaryPhone}`} className="font-semibold text-primary hover:underline">{SCHOOL.primaryPhoneDisplay}</a> or email <a href={`mailto:${SCHOOL.email}`} className="font-semibold text-primary hover:underline">{SCHOOL.email}</a> for detailed information</li>
            </ul>
          </div>
        </div>
      </section>
